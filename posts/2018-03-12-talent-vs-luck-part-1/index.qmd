...
What makes the model's outcome inevitable is its multiplicative structure. While the *average* capital grows slightly for talented people, the *typical* (median) outcome is a loss for everyone — even at the 99.9th percentile of talent. A few lucky individuals capture enormous gains, pulling the mean up, while most people lose capital regardless of how talented they are. Only someone with literally perfect talent ($T = 1$) breaks even on the median. This asymmetry is baked into the model by construction: talent can help you capitalize on lucky events, but it offers zero protection against unlucky ones.
...
## Mean vs Median: The Multiplicative Trap

Because we have analytical probabilities, we can compute both the expected (mean) and typical (median) final capital exactly for any talent quantile. No simulation needed.

The expected value of each timestep multiplier is $E[m] = 2 p_\text{double} + 1 \cdot p_\text{same} + 0.5 \cdot p_\text{halve}$, which is above 1 for talent > 0.5. But capital is a *multiplicative* process — what matters for the typical person is the *geometric* mean: $\exp(p_\text{double} \ln 2 + p_\text{halve} \ln 0.5)$. The geometric mean is below 1 for all talent < 1.

```{python}
#| label: mean-vs-median
#| code-summary: "Mean and median capital calculations"
...
def median_value(quantile, starting_capital=STARTING_CAPITAL, n_timesteps=N_TIMESTEPS):
    p_halve, p_same, p_double = tvl_probabilities(quantile)
...
for q, label in [(0.5, "50th"), (0.95, "95th"), (0.999, "99.9th"), (0.99999, "99.999th")]:
    ev = expected_value(q)
    med = median_value(q)
    print(f"{label:10s} percentile → mean: {ev:8.2f}   median: {med:7.4f}  (started at {STARTING_CAPITAL})")
```

The mean and median tell completely different stories. The mean grows — a 95th-percentile person has an expected final capital over 50. But the *median* shrinks for every talent level below perfect: a 95th-percentile person's typical outcome is about 3.5, down from 10. Only someone with literally perfect talent ($T = 1$) breaks even on the median.

This is the signature of a multiplicative process. A few people get lucky runs — several doublings without any halvings — and their enormous gains pull the mean far above the median. The mean is dominated by rare winners; the median reflects what happens to most people.

```{python}
#| label: fig-mean-vs-median
#| fig-cap: "Mean (dashed) vs median (solid) final capital after 80 timesteps. The mean grows with talent, but the median — the typical outcome — stays well below starting capital for all realistic talent levels. Only perfect talent (T = 1) breaks even on the median."
quantiles = np.linspace(0.001, 0.999, 300)

fig, ax = plt.subplots(figsize=(7, 5))
evs = [expected_value(q) for q in quantiles]
meds = [median_value(q) for q in quantiles]
ax.plot(quantiles, evs, ls="--", lw=2, color="#2196F3", label="Mean (expected value)")
ax.plot(quantiles, meds, ls="-", lw=2, color="#F44336", label="Median (typical outcome)")
